...
function Resolve-ReferenceTestHandler {
    param(
        [Parameter(Mandatory = $true)]
        [string]$CaseId
    )
    ...
    $candidates = @(
        "Invoke-ReferenceCase${CaseId}Test",
        "Test-ReferenceWalkthroughCase${CaseId}"
    )
    ...
    throw "No test handler was found for case $CaseId"
}

function Resolve-ReferenceOptionalSetupHandler {
    param(
        [Parameter(Mandatory = $true)]
        [string]$CaseId
    )
...
foreach ($caseId in $context.Cases) {
    try {
        $setupState = $null
        $setupResult = $null
        $setupHandler = Resolve-ReferenceOptionalSetupHandler -CaseId $caseId
        if ($setupHandler) {
            $setupState = & $setupHandler -Context $context
            $setupResult = Normalize-ReferenceResult $setupState
        }

        $handler = Resolve-ReferenceTestHandler -CaseId $caseId
        $command = Get-Command $handler
        $invokeArgs = @{ Context = $context }
        if ($setupState -and $command.Parameters.ContainsKey('SetupState')) {
            $invokeArgs['SetupState'] = $setupState
        }
        if ($setupResult -and $command.Parameters.ContainsKey('SetupResult')) {
            $invokeArgs['SetupResult'] = $setupResult
        }

        $result = Normalize-ReferenceResult (& $handler @invokeArgs)
        $results.Add($result)
    }
    catch {
        $results.Add((New-ReferenceResult -CaseId $caseId -Stage 'test' -Status 'BLOCKED' -Summary $_.Exception.Message))
    }
}

$summaryReport = Write-ReferenceSummaryReport -Context $context -Results $results.ToArray() -Name 'test-summary'
Write-Host "Preflight report: $preflightReport"
Write-Host "Test summary: $summaryReport"
